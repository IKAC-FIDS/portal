...
{
    public class InqueryFromPardakhtNovinTask : ScheduledTaskTemplate
    {
            ...
            using (var dataContext = new AppDataContext())
            {
                var terminalData = await dataContext.Terminals.Include(b=>b.TerminalDocuments)
                    ...
                    .Where(x => x.PspId == (byte) PspCompany.PardakhNovin
                      ...
                      && ( x.StatusId == (byte) TerminalStatus.NotReturnedFromSwitch || 
                           x.StatusId == (byte) TerminalStatus.NeedToReform )
                    )
                    .Select(x => new
                    {
                        x.Id,
