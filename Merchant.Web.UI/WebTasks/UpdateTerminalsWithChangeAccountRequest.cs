...
{
    public class UpdateTerminalsWithChangeAccountRequest : ScheduledTaskTemplate
    {
        ...
        public override async Task RunAsync()
        {
            ...
            using (var dataContext = new AppDataContext())
            {
                ...
                var pardakhNovinData = changeAccountRequestsInfo.Where(a =>  !string.IsNullOrEmpty(a.PardakhtNovinTrackId));

                if (pardakhNovinData.Any())
                {
                    using (var pardakhtNovinService = new PardakhtNovinService())
                    {
                        foreach (var request in pardakhNovinData)
                        {
                           try
                            {
                                var inqueryResult =   pardakhtNovinService.ChangeAccountInquery( request.Id,request.PardakhtNovinTrackId);
                                if (inqueryResult != null && inqueryResult.Status == PardakthNovinStatus.Successed
                                                          && inqueryResult.Data.WorkFlowValue == "EndOfFlow")
                                {
                                    successfullCount++;
                                    
                                    await dataContext.ChangeAccountRequests
                                        .Where(x => x.TerminalNo == request.TerminalNo &&
                                                    x.ShebaNo == inqueryResult.Data.NewShabaCode).UpdateAsync(x =>
                                            new Data.Domain.ChangeAccountRequest
                                                { StatusId = Enums.RequestStatus.Done.ToByte() });
                                        
                                    await dataContext.Terminals.Where(x => x.TerminalNo == request.TerminalNo)
                                        .UpdateAsync(x => new Terminal
                                        {
                                            AccountNo = inqueryResult.Data.NewAccountNumber, ShebaNo = inqueryResult.Data.NewShabaCode
                                        });
                                    
                                    
                                }
                                else if (inqueryResult != null &&
                                         inqueryResult.Status == PardakthNovinStatus.Successed &&
                                         inqueryResult.Data.WorkFlowValue == "ShaparakError")
                                {
                            ...
                            {
                                faildTerminalIdList.Add(request.Id);
                                exception.AddLogData("TerminalId", request.Id).LogNoContext();
                            }
                        }
                    }
                }
                ...
                if (irankishData.Any())
                {
                    using (var irankishService = new IranKishService())
                    {
                        foreach (var terminalInfo in irankishData)
                        {
                            try
                            {
                                var inqueryResult =   irankishService.TryInqueryAcceptor(terminalInfo.TerminalNo,
                                    terminalInfo.Id, terminalInfo.StatusId).Result;
                                if (inqueryResult != null && inqueryResult.IsSuccess)
                                {
                                    successfullCount++;
                                    if (inqueryResult.ShebaNo == terminalInfo.ShebaNo)
                                    {
                                        var m = dataContext.ChangeAccountRequests
                                            .Where(x => x.TerminalNo == terminalInfo.TerminalNo &&
                                                        x.ShebaNo == terminalInfo.ShebaNo).FirstOrDefault();
                                        m.StatusId =   Enums.RequestStatus.SentToPsp.ToByte();
                                        var k = dataContext.SaveChanges();
                                         var t = k;
                                    }
                                    else
                                    {
                                        var requestedShebaNo = changeAccountRequestsInfo
                                            .Where(x => 
                                                x.TerminalNo == terminalInfo.TerminalNo).Select(x => x.ShebaNo)
                                            .FirstOrDefault();
                                        if (!string.IsNullOrEmpty(requestedShebaNo) &&
                                            requestedShebaNo == inqueryResult.ShebaNo)
                                        {
                                            await dataContext.Terminals.Where(x => x.Id == terminalInfo.Id)
                                                .UpdateAsync(x => new Terminal
                                                {
                                                    AccountNo = inqueryResult.AccountNo, ShebaNo = inqueryResult.ShebaNo
                                                });
                                            await dataContext.ChangeAccountRequests
                                                .Where(x => x.TerminalNo == inqueryResult.TerminalNo &&
                                                            x.ShebaNo == inqueryResult.ShebaNo).UpdateAsync(x =>
                                                    new Data.Domain.ChangeAccountRequest
                                                        { StatusId = Enums.RequestStatus.Done.ToByte() });
                                        }
                                    }
                        ...
                        foreach (var terminalInfo in parsianData)
                        {
                            try
                            {
                                ...
                                if (requestId.HasValue)
                                {
                                    ...
                                    var result =
                                        parsianService.RequestInQuery(requestInqueryInput, (int)terminalInfo.Id);
                        ...
                        foreach (var terminalInfo in fanavaData)
                        {
                            try
                            {
                                ...
                                if (result.IsSuccess)
                                {
                                    ...
                                    {
                                        await dataContext.ChangeAccountRequests
                                            .Where(x => x.TerminalNo == result.TerminalNo &&
                                                        x.ShebaNo == result.ShebaNo).UpdateAsync(x =>
                                                new Data.Domain.ChangeAccountRequest
                                                    { StatusId = Enums.RequestStatus.Done.ToByte() });
                                    }
                                    else
                                    {
                                        var requestedShebaNo = changeAccountRequestsInfo
                                            .Where(x => x.TerminalNo == terminalInfo.TerminalNo).Select(x => x.ShebaNo)
                                            .FirstOrDefault();
                                        if (!string.IsNullOrEmpty(requestedShebaNo) &&
                                            requestedShebaNo == result.ShebaNo)
                                        {
                                            await dataContext.Terminals.Where(x => x.Id == terminalInfo.Id)
                                                .UpdateAsync(x => new Terminal
                                                    { AccountNo = result.AccountNo, ShebaNo = result.ShebaNo });
                                            await dataContext.ChangeAccountRequests
                                                .Where(x => x.TerminalNo == result.TerminalNo &&
                                                            x.ShebaNo == result.ShebaNo).UpdateAsync(x =>
                                                    new Data.Domain.ChangeAccountRequest
