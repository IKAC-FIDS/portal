...
{
    public class MessageReplyDocumentController  : BaseController
    {
        private readonly AppDataContext _dataContext;

        public MessageReplyDocumentController(AppDataContext dataContext)
        {
        ...
        {
            var query = _dataContext.MessageReplyDocuments.Where(x => x.Id == id);

            if (!User.IsAdmin() && !User.IsMessageManagerUser())
            {
                if (User.IsAcceptorsExpertUser())
                {
                    query = query.Where(x => x.MessageReply.Message.ReviewerUserId == CurrentUserId);
                }
                else
                {
                    query = query.Where(x => x.MessageReply.Message.UserId == CurrentUserId);
                }
            }
